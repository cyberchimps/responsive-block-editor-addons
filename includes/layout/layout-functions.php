...
use RBEA\Blocks\Layouts\Component_Registry;
 ...
 */
function rbea_blocks_register_layout_component( array $data ) {
 ...
 */
function rbea_blocks_unregister_layout_component( $type, $key ) {
	$registry = Component_Registry::instance();
	try {
 ...
 */
function rbea_blocks_get_layout_component( $type, $key ) {

	if ( empty( $type ) ) {
		return new WP_Error( esc_html__( 'You must supply a type to retrieve a layout component.', 'responsive-block-editor-addons' ) );
	}

	if ( empty( $key ) ) {
		return new WP_Error( esc_html__( 'You must supply a key to retrieve a layout component.', 'responsive-block-editor-addons' ) );
	}
 ...
 */
function rbea_blocks_get_layouts() {
	$registry = Component_Registry::instance();
 ...
 */
function rbea_blocks_get_sections() {
	$registry = Component_Registry::instance();
